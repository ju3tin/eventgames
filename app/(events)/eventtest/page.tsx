...
import { IDL } from './idl'; // Make sure your IDL is exported here

// ================== DEVNET CONFIG ==================
const PROGRAM_ID = new PublicKey('7mCaQvGKDicYCH2ruxF6uD9W8QJpk4hE2cWLmimU8iuT');
const DEVNET_RPC = 'https://api.devnet.solana.com';

// Devnet USDC Mint (official test USDC)
const USDC_MINT = new PublicKey('4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU');

const TOKEN_PROGRAM_ID = new PublicKey('TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA');
const ASSOCIATED_TOKEN_PROGRAM_ID = new PublicKey('ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL');

// Common seeds used in many Anchor programs (adjust if your Rust code uses different seeds)
const VAULT_AUTHORITY_SEED = Buffer.from('vault_authority');
const ESCROW_VAULT_SEED = Buffer.from('escrow_vault');

export default function CreateChallengePage() {
  const { connection } = useConnection(); // This will be overridden with devnet below
  ...
  const [formData, setFormData] = useState({
    ...
    finishDate: '',
    finishTime: '',
    entryFee: '1000000',        // 1 USDC (assuming 6 decimals)
    maxParticipants: '100',
  });
    ...
    try {
      ...
      const txSignature = await program.methods
        .createChallenge(
          ...
          new BN(startTs),
          new BN(0),           // startTime (if not used)
          new BN(finishTs),
          new BN(0),           // finishTime (if not used)
      ...
      setStatus({
        type: 'success',
        message: `Challenge created successfully on Devnet! Transaction: ${txSignature}`
      });
    ...
    <div className="min-h-screen bg-gray-950 text-white p-8">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-yellow-900/30 border border-yellow-600 text-yellow-300 p-4 rounded-xl mb-6 text-sm">
          ...
          • Get test USDC from Circle faucet or request from someone<br />
        ...
        <form onSubmit={createChallenge} className="space-y-6 bg-gray-900 p-8 rounded-2xl border border-gray-800">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="date" name="startDate" value={formData.startDate} onChange={handleChange} required className="w-full px-4 py-3 bg-gray-800 rounded-lg" />
            </div>
            <div>
              <label className="block text-sm mb-2">Start Time</label>
              <input type="time" name="startTime" value={formData.startTime} onChange={handleChange} required className="w-full px-4 py-3 bg-gray-800 rounded-lg" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="date" name="finishDate" value={formData.finishDate} onChange={handleChange} required className="w-full px-4 py-3 bg-gray-800 rounded-lg" />
            </div>
            <div>
              <label className="block text-sm mb-2">Finish Time</label>
              <input type="time" name="finishTime" value={formData.finishTime} onChange={handleChange} required className="w-full px-4 py-3 bg-gray-800 rounded-lg" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" name="entryFee" value={formData.entryFee} onChange={handleChange} required className="w-full px-4 py-3 bg-gray-800 rounded-lg" />
            ...
            <div>
              ...
              <input type="number" name="maxParticipants" value={formData.maxParticipants} onChange={handleChange} required className="w-full px-4 py-3 bg-gray-800 rounded-lg" />
            </div>
          </div>
        ...
        {status && (
          <div className={`mt-6 p-5 rounded-2xl text-sm ${
            status.type === 'success' ? 'bg-green-900/50 border border-green-700' : 
            status.type === 'error' ? 'bg-red-900/50 border border-red-700' : 'bg-blue-900/50'
          }`}>
            {status.message}
          </div>
        )}
